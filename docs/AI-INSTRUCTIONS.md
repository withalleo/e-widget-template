...
> **Fetching URLs and files:** the iframe is served from a sandboxed origin, so a plain `fetch()` to a third-party URL is blocked. You **must always prefer** `alleo.fetchProtectedUrlJSON` / `fetchProtectedUrlText` / `fetchProtectedUrlBinary` over the native `fetch()` when retrieving anything from the internet - never a plain `fetch()`. Pass `keyId: null` for a **public, unauthenticated** URL or file; this is the same as an authenticated call but without attaching any credential. If the API requires authentication, pass the real `keyId` string instead (see the SDK reference below) - that is the **only** way to reach an authenticated third-party API from inside the iframe.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------- | ------- |
| `alleo.fetchProtectedUrlJSON` / `fetchProtectedUrlText` / `fetchProtectedUrlBinary` (both the `keyId: null` case and authenticated calls with a real `keyId`) | ✅    | ❌      | ❌      |
...
**This is the only supported way to fetch anything from the internet - always prefer it over the native `fetch()`.** A plain `fetch()` to a third-party origin will always fail. `keyId` controls which mode is used:

- **`keyId: null`** - fetches a **public, unauthenticated** URL or file (JSON APIs, RSS/Atom feeds, images, text, etc.). Use this whenever the request does **not** carry a secret, token, password, or API key.
...
```js
/**
 * @param {string | null} keyId - null for a public/unauthenticated fetch, or a configured backend keyId for an authenticated call
...
- Requires `enableIframeCommunication: true` and `enableBackendProxy: true` for both the `keyId: null` and authenticated cases. An authenticated call additionally needs a matching `allowedProtectedBackends[keyId]` entry in the footer with the **exact endpoint URL (s)** the widget calls in `allowedUrls` - see [Importable settings footer](#importable-settings-footer) for the required schema format. A `keyId: null` call additionally needs the domain's common URL prefix listed in the footer's `allowedPublicUrls` (one prefix per distinct domain, not every exact URL). Right after the import, Alleo asks the board owner to enter the real credential for each declared connection (it can also be set later from the widget's "Configure External Connection" context menu action); the footer only declares which endpoints are allowed, never the secret value.
...
- Include **only** keys from the allowed list below - any other key is dropped on import. Never emit `htmlContent`, `sourceType`, `url`, `fileId`, `boardObjectWhitelist`, or `newContentContainer` - those are derived on import, unsupported, or chosen by the user in the import dialog.
- **Every key shown in the footer above is required** - include all of them in every footer with an explicit value. Set unused features to their "off" value (`false` / `[]` / empty); never omit them. Enable a flag (`true`) only when the HTML actually uses that feature. The remaining keys (`allowedProtectedBackends`, `allowedPublicUrls`) stay optional and are added only when relevant.
...
| `enableGetParams`                    | `boolean`                                                                                                                                                                  | `false`              | the HTML reads `alleo.getParams`. Requires `enableIframeCommunication: true`.                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `allowedProtectedBackends`           | `Record<string, { backendSchemas: [{ method: "QueryParam" \| "Header", template: string, connection: "Http" \| "WebSocket", allowedUrls: [string, ...string[]] }, ...] }>` | `{}` (optional)      | one entry per non-null `keyId` passed to `alleo.fetchProtectedUrl*(keyId, ...)`. Not needed for calls that pass `keyId: null` (public content fetch). **`allowedUrls` MUST list the exact endpoint(s) the HTML calls for that `keyId`** - this is the security boundary that stops the widget from being turned into an open proxy; never leave it empty, omit it, or use a bare/wildcard domain. See [Protected backend connections](#protected-backend-connections-footer) below for the full format. |
| `allowedPublicUrls`                  | `string[]`                                                                                                                                                                 | `[]` (optional)      | **Required whenever the HTML calls `alleo.fetchProtectedUrl*(null, ...)`** (public/unauthenticated fetch). List one common URL prefix per distinct domain fetched this way (e.g. `"https://api.example.com/"`) - not every exact URL called. Not needed if the widget never uses `keyId: null`.                                                                                                                                                                                                         |
...
| `requiredAPIfunctions`               | `string[]`                                                                                                                                                                 | `[]`                 | **always required.** List every `alleo.*` endpoint the HTML actually uses, without the `alleo.` prefix - this includes function calls **and** property reads like `alleo.getParams`/`alleo.user`, not just functions (e.g. `["initialize", "triggerAction", "onIncomingAction", "fetchProtectedUrlJSON", "getParams", "user"]`). Keep it in sync with the HTML - add an entry whenever you add a new `alleo.*` reference, remove it if the reference is removed.                                        |
...
Set `enableBackendProxy: true` whenever the widget calls `alleo.fetchProtectedUrlJSON` / `fetchProtectedUrlText` / `fetchProtectedUrlBinary` for **any** reason - including a public, unauthenticated fetch with `keyId: null`. **Always prefer this over a plain `fetch()` to download anything from the internet.** Additionally add one `allowedProtectedBackends` entry for every non-null `keyId` used - **this is the only supported way for a generated widget to reach an API that requires an API key, bearer token, or any other credential.** Never invent an alternative (embedding the key in the HTML, asking the user to paste it into a form field you control, routing an authenticated call through the `keyId: null` path, etc.).
...
- `<keyId>` must exactly match the string passed as the first argument to every authenticated `alleo.fetchProtectedUrl*(keyId, ...)` call in your HTML for that connection. Calls that pass `keyId: null` (public content fetch) don't need an entry here.
- `backendSchemas` is an array - use a single entry unless the same credential really has to be attached in more than one way (e.g. header for one endpoint, query parameter for another). A legacy single `backendSchema` object is still accepted on import, but always generate `backendSchemas`.
...
- **`allowedUrls` is mandatory and is a hard security boundary - it is not optional decoration.** It must list the **exact** endpoint URL (s) the HTML actually calls for that `keyId`. Never omit it, leave it empty, or set it to a bare/wildcard domain "just in case" - any request to a URL outside this list is rejected before the credential is ever attached, an entry with no allowed endpoint at all blocks every request made with that `keyId`, and an overly broad list defeats the entire point of the feature.
- The footer **never** contains the actual secret/API key value - only the schema describing which endpoints are allowed and how the credential (entered later by a human) gets attached. Right after the import, Alleo asks the board owner to enter the credential for each declared connection; it can also be entered or changed later from the widget's context menu via **"Configure External Connection `<keyId>`"**.

For every `keyId: null` (public, unauthenticated) call, add the domain's common URL prefix to the footer's `allowedPublicUrls` array instead - one prefix per distinct domain fetched from, not every exact URL:
...
7. `alleo.getBoardObjectContent`/`replaceBoardObjectContent`/`appendBoardObjectContent` → `enableIframeFuncBoardObjectContent: true` (never emit `boardObjectWhitelist` - the user picks the objects in the import dialog).
...
11. Any `alleo.fetchProtectedUrlJSON`/`fetchProtectedUrlText`/`fetchProtectedUrlBinary` call (public `keyId: null` fetch **or** authenticated `('<keyId>', ...)` call) → `enableBackendProxy: true`. For an authenticated call, also add one `allowedProtectedBackends['<keyId>']` entry with a correctly-formatted `backendSchemas` array, **`allowedUrls` listing the exact endpoint (s) called for that `keyId`** (see [Protected backend connections](#protected-backend-connections-footer)). For a `keyId: null` call, also add the domain's common URL prefix to `allowedPublicUrls` (one prefix per distinct domain, not every exact URL). This is the only way to fetch anything from the internet or reach an authenticated API - never invent an alternative.
...
14. Collect every distinct `alleo.*` reference found in the HTML (from steps 2–11 above and any others, e.g. `alleo.initialize`) into `requiredAPIfunctions`, listing each name **without** the `alleo.` prefix. This includes property reads such as `alleo.getParams` and `alleo.user`, not just function calls.
...
- [ ] All URL/file fetches use `alleo.fetchProtectedUrlJSON`/`fetchProtectedUrlText`/`fetchProtectedUrlBinary` instead of a plain `fetch()` - always prefer these (with `keyId: null`) for public resources, and the real `keyId` for anything requiring an API key/token/credential; never a plain `fetch()`, and never a hard-coded secret.
...
- [ ] The footer contains only allowed keys - no `htmlContent`, `sourceType`, `url`, `fileId`, `overwriteTextColor`, `boardObjectWhitelist`, or `newContentContainer`.
...
- [ ] If the HTML calls `alleo.fetchProtectedUrlJSON`/`fetchProtectedUrlText`/`fetchProtectedUrlBinary` (public `keyId: null` fetch or authenticated call), `enableBackendProxy: true` is set; for each non-null `keyId` used, `allowedProtectedBackends` has one correctly formatted entry with a non-empty, exact/narrow `allowedUrls` list - never omitted, empty, or wildcarded. For each `keyId: null` call, `allowedPublicUrls` lists the common URL prefix of every distinct domain fetched publicly - never omitted when a public fetch is used.
- [ ] The footer's `requiredAPIfunctions` lists every `alleo.*` endpoint actually used in the HTML (without the `alleo.` prefix) - including property reads like `getParams`/`user`, not just function calls - and contains no unused entries.
