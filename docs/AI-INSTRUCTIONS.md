<!--
  ...
  agent works autonomously and writes the widget to `dist/<name>.txt`. Use this
  file for the deeper rationale and the worked end-to-end example.
-->

# AI Instructions — E-widget HTML Content
...
Every class must begin with a clearly marked **configuration block** that groups all tunable values together. This makes
the widget easy to customize without reading through the entire codebase.
...
Place a commented-out HTML block at the very top of the generated file. It must list every outgoing action, every
incoming action (with their parameter names and types), and any warnings.
...
- `onloaded` **must always be the first outgoing action listed.** Every widget fires it exactly once after full
  initialization.
- Include the `⚠ WARNING` lines only when the content genuinely requires those capabilities.
- Omit sections that have no entries (e.g., omit "Incoming actions" if there are none, omit "Board object content" if
  the widget does not use it).
...
- [ ] Summary comment at the top lists all outgoing and incoming actions with parameters.
- [ ] Summary comment lists board object content operations (whitelist indices used) if applicable.
...
- [ ] `onloaded` outgoing action is fired exactly once after full initialization and is listed first in the summary
  comment.
...
- [ ] Widget logic is organized into classes with descriptive names — no loose top-level functions or unstructured
  procedural code.
- [ ] Configurable values are grouped at the top of the class in a marked `/* ═══ Configuration ═══ */` section with
  JSDoc comments.
